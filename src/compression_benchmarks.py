...
def test_compression_ratio_vs_chunksize_finegrained(
    z_arr, min_chunksize=100, max_chunksize=256, dim="sample", dry_run=False
):
    ...
    for i in tqdm(
        range(min_chunksize, max_chunksize + 1), total=max_chunksize - min_chunksize
    ):
        ...
        if not dry_run:
            z2 = zarr.empty_like(
                arr, chunks=chunksize, compressor=z_arr.compressor, dtype=z_arr.dtype
            )
            z2[:] = arr
            nchunks = z2.nchunks
            compress_ratio = float(dict(z2.info_items())["Storage ratio"])
...
def test_vcf2zarr_compression_variations(
    ...
    for k in keys:
        ...
        if test_config == "chunksize":
            ...
            if k != "call_genotype":
                continue
            res = test_compression_ratio_vs_chunksize_finegrained(
                z_group[k], dry_run=dry_run
            )
        else:
