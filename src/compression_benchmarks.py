...
def generate_chunksize_variations(default_variant_chunksize=10_000,
                                  default_sample_chunksize=1_000):
...
def test_compression_ratio_vs_chunksize(z_arr,
                                        dry_run=False):
    """
    ...
    for cs_var in tqdm(chunksize_var,
                       total=len(chunksize_var),
                       desc='Chunksizes'):
        ...
        comp_results_table.append({'chunksize': cs_var,
...
def test_compression_ratio_vs_shuffle(z_arr,
                                      dry_run=False):
    ...
    for sv, sn in tqdm(zip(shuffle_var, shuffle_names), desc='Testing shuffles', total=3):
        ...
        comp_results_table.append({'Shuffle': sn,
                                    'nchunks': n_chunks,
                                    'CompressionRatio': compress_ratio
                                    })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-config', dest='test_config', type=str, default='chunksize',
