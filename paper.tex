...
and metadata for a single variant~\citep{danecek2011variant} are
encoded as a line of text.
BCF (a packed binary representation) is similarly 
...
optimised for ``tall and narrow'' data there is a large number of records
...
VCF is at its core an encoding of the genotype matrix $G$, where each entry
encodes the observed genotypes for a given sample at a give variant site,
interleaved with per-variant information
and other matrices (e.g., GQ or AD).
The data is largely numerical and of a fixed dimension, and is therefore a 
natural mapping to an array-oriented approach [PICK UP HERE]
