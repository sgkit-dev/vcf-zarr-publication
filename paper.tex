...
standard practice, which warrant some discussion here.

Perhaps the largest deviation in the approach that we advocate here 
from standard practice is to decouple our concept of a dataset from
...
and vcf2zarr 0.0.9. All tools used default settings, 
...
\texttt{time}) as well as elapsed wall-clock time. 
Each tool was instructed to use one thread, where the options
were provided. 
Where possible in pipelines we use uncompressed BCF
output \texttt{-Ou} to make processing more efficient~\citep{danecek2021twelve}.
