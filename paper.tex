...
most notably the highly-influential Positional
...
row-wise storage can inefficient because we will usually need to
...
instead of grouping toether all the fields for a record,
...
SeqArray~\citep{zheng2017seqarray} built on 
the GDS container format~\citep{zheng2012high}.
...
Note that this approach to storing higher dimensional 
...
[TODO check these citations]~\cite[e.g.][]{lefaive2021sparse,
wertenbroek2022xsi,zhang2023gbc}.
...
on the data of Figure~\ref{fig-data-storage}. We can see 
that for $10^6$ samples, running \texttt{bcftools +af-dist}
requires 20 hours (note that this is run on a BCF file, for 
efficiency), which can be regarded as a reasonable baseline.
Because genozip does not provide an API, we used the classical
approach of piping the VCF text that it outputs into 
\texttt{bcftools +af-dist}. The volumes of data involved here
make this an expensive approach.
[TODO discuss Zarr-python and Savvy C++ implementations]
