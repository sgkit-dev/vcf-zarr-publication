...
such that the allele 0 is always A, 1 is C etc.
...
overall storage (see Table~\ref{tab-genomics-england-data} for column details).
Field names (derived from source CSV file) are shortened for 
...
arrays, and shows that the alignment data takes up 298 MiB. 
This is around 81X smaller than gzipped FASTA.
...
However, these files cannot be accessed in-situ and must be decompressed
in order to access the data.

Computational accessibility for the Zarr is excellent. 
Retrieving the full alignment for the 1 millionth sample required
[x ms]. Performing the same operation on the corresponding indexed FASTA
file using pyfaidx~\citep{shirley2015efficient} required [x ms].
Similarly, accessing calls for all samples 
at the 10,000th site requires xms
The storage of metadata and genetic data in a single file also
...
(SAIGE reading data from PLINK, BGEN, VCF, BCF and SAV formats) 
...
from Fig~\ref{fig-data-storage}. We performed a single-variant association
test based on a phenotype simulated using
\texttt{tstrait}~\citep{tagami2024tstrait},
and measure the overall time required when using the 
equivalent VCF, BCF, SAV and Zarr files.

We compared the same analysis using the existing VCF, BCF and Savvy backends
