...
in the UKB WGS data~\citep{uk2023whole} is 116 TiB.
...
is quality control (QC). Variant calling QC is an 
...
Genotype calls are often further refined via statistical methods,
% methods~\citep[e.g.][MORE and CHECK]{browning2021fast,browning2023statistical}
for example by phasing~[CITES] and imputation[CITES], usually 
creating an additional copy of the dataset.
...
% Can easily delete this last bit:
effectively making 
analysis of these datasets the preserve of a well-funded elite.

To fully utilise these vast variation datasets,
we require a primary data representation that can 
support different types of query without 
...
it must also \emph{accessible} using protocols that are 
...
Zarr's elegant simplicity and direct mapping of chunks to 
