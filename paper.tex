...
In principle, Zarr can store data into any key-value store.
...
% Good points from  the new Horizons technical paper:

% From a technical standpoint, a standout feature of Zarr is its simplicity.
% The format is easy for developers to
% understand because the internals are exposed transparently. Consequently, Zarr
% is considered easy to “hack,”
% i.e. to adapt to new use cases and scenarios, such as implementing a reader
% from a new programming language
% or creating a new backend storage layer. (This is a significant contrast from
% NetCDF4 / HDF5, whose internal file
% format is relatively opaque, and for which all access must go through the
% official C library.)

% The Zarr project is governed by a steering council of contributors from
% different institutions. No single company or institution controls the
% specification and its implementations.
% Second, although Zarr is relatively new, it is currently being evaluated by the
...
Zarr is being considered a potential format for migrating the 
Copernicus global land service to the cloud~\citep{abernathey2021opening}
...
CMIP6 dataset in Google Cloud~\cite{cmip6_dataset}
