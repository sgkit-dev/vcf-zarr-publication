...
and a more scalable, grid-based solution is needed.
...
Large genome variation datasets are also being generated for other organisms
...
task~\cite{marees2018tutorial,panoutsopoulou2018quality,chen2024genomic}, 
...
(i.e., consisting of tightly coupled components)
...
and works well when records are relatively small, 
or when we typically want to analyse each record in its entirety.
...
Columnar technologies tend to be 
...
Additionally, compression performance within columns tends to
be poor because there is much less similarity among
the genotypes for a given sample than there is among the genotypes
for a given variant, requiring
specialised haplotype compression 
techniques~\citep{layer2016efficient,mcvean2019linkage}.
...
Thirdly, Zarr is not suitable for handling 
the unstructured data often included in VCF INFO 
fields, and would likely be a poor choice
for representing variant annotations, for example.
...
as required.
Even finer granularity is possible if samples are grouped according
to access level within chunks (i.e, by padding partially filled 
chunks and using an appropriate sample mask). 
Delegating access control to the cloud provider in this way
and providing client applications direct access to
the data in an efficiently analysable form, 
largely negates the 
need for web API access~\cite{kelleher2019htsget} 
and cryptographic container formats~\citep{senf2021crypt4gh}
in this setting.

The draft VCF Zarr specification and scalable \texttt{vcf2zarr} conversion utility
...
Two initial directions for development, however, may provide 
a sufficient starting point to both
greatly improve researcher productivity on 
large, centrally managed datasets such as Genomics England
and motivate further development.
...
The second direction is then to develop new methods 
working directly with the Zarr data, and can leverage 
a rich ecosystem of powerful and efficient tools in 
multiple programming languages (see Methods).
...
kluyver2016jupyter,
virtanen2020scipy} 
and multiple Zarr implementations (see Methods).
Of particular interest for large-scale datasets are
Dask~\cite{rocklin2015dask} and 
Xarray~\citep{hoyer2017xarray} which provide powerful abstractions
for distributed computing and can be
straightforwardly used to perform interactive analysis
of VCF Zarr datasets,
with computations transparently distributed over a cluster.
As shown in our benchmarks, lower-level computations
in which numerical operations are applied on a chunk-by-chunk
basis are also straightforward to implement and highly performant.
Thus, a range of possibilities exist in which developers can 
choose the appropriate level of abstraction and tool chain
on a case-by-case basis.

[NOTE: still trying to figure out how to string this together. 
Want to finish on a positive, not too technical note.]

and is gaining increasing
...
developed to store data from the \textit{Anopheles gambiae} 
1000 Genomes Project~\citep{anopheles2017genetic}.
A successor project, the MalariaGEN Vector Observatory,
...
democratising the use of data by scientists in malaria-endemic countries
...
Interestingly, the choice chunk size along the sample dimension for the
genotype matrix does a significant effect. 
