...
describes the observed genotypes for a given sample at a give variant site,
interleaved with per-variant information
and other matrices (e.g., the GQ or AD fields).
The data is largely numerical and each field is of a fixed dimension, 
and is therefore a natural mapping to array-oriented 
or ``tensor'' storage.
We propose the VCF Zarr specification which maps the 
VCF data model into an array-oriented layout using the
