...
VCF's simple text-based design and widespread support makes it an 
excellent archival format, but it is a poor basis for computational
workflows. 
% Methods that require efficient access to genotype data
% either require convertion to the
...
few broad categories. The first level of analysis that is 
done is quality control (QC). Variant calling QC is an 
involved and multi-faceted 
task [CITATIONS?], requiring analysis of many different metrics
and incurring substantial computation.
...
Hail distributes data and calculations over a cluster, enabling
interactive analysis of very large datasets. It has been successfully used 
in many studies, and instrumental in datasets such as 
gnomadAD~\cite{karczewski2020mutational,chen2024genomic}.
After QC, a common task is to perform a genome wide association study
(GWAS)~\cite{uffelmann2021genome}. The majority of tools for 
performing GWAS and associated tasks require
data to be in PLINK format~\cite[e.g][]{chang2015second} [MORE],
and so data must be ``hard-called'' according to some QC criteria
and exported to additional files.
...
% '115.7 TiB'
Many applications examine data on a variant-by-variant
basis, but others,
e.g.\ reference-panel based phasing and imputation 
methods~\citep[e.g.][MORE and CHECK]{browning2021fast,browning2023statistical}
% and Ancestral Recombination Graph 
% (ARG) inference~\cite{brandt2024promise,lewanski2024era,wong2024general}
require access on a sample-by-sample basis.
...
study (see section XXX).

VCF (and BCF) cannot support any of these workflows efficiently
at the Biobank scale, and workarounds of creating subsets 
of data (i.e., discarding QC metrics in VCF) or copying to 
PLINK format are becoming less effective.
...
The analysis of Biobank-scale variation 
datasets requires a primary data representation that can 
efficiently support different types of query without 
conversion or intermediate copies, and operate 
directly from cloud object stores. Such a representation
can be termed ``analysis-ready''
and ``cloud-native''~\citep{abernathey2021cloud}.
For such a representation to be FAIR~\citep{wilkinson2016fair},
it must also \emph{accessible}: ``open, free, and universally
implementable''.
There is no currently no efficient, FAIR representation of genetic variation
data suitable for cloud deployments.
While Hail is powerful software its file formats are not documented or 
designed for reuse.
...
a method of storing large multidimensional data as a regular
grid of compressed chunks. 
Zarr's simplicity and its direct mapping of chunks to 
keys in an object store have lead to 
...
VCF (and BCF), and is competitive with state-of-the-art
VCF compression tools. 
...
and metadata for a single variant~\citep{danecek2011variant}. 
VCF is therefore a \emph{row-wise} format, i.e., where all of the 
data for a record is stored continguously. BCF (a packed binary 
representation) is similarly 
...
makes retrieving particular columns much 
...
use of technologies such as Parquet for 
storing and analysing variation data have had limited
success~\citep{boufea2017managing,fan2020variant}.
Directly mapping VCF data to a columnar layout, in which there is a 
a column for the genotypes (and other per-sample QC metrics) 
for each sample, scales poorly. Firstly, it leads to a large number 
of columns (millions, at Biobank scale), which techologies such 
as Parquet are not designed for. Secondly, compression performance 
within columns is poor because there is much less similarity among
...
[TODO intro passage here on array-based or ``tensor'' storage. Separate
out the different interleaved matrices with the VCF, and store them
in regular chunked grids.]
