...
first convert the X VCF files (YTiB) for chromosome 2
into the intermediate columnar format. This task was 
distributed over a cluster of N workers, using a 
total of X CPU hours and an elapsed wall-clock time 
of X hours. When then converted the intermediate 
column format to Zarr using 
SOMETHING which took X hours using [RESOURCES]. 
This produced a dataset with N arrays, consuming a 
total of Y TiB of storage over X directories and 
Y files. The top fields in terms 
of storage are detailed in Table~\ref{tab-genomics-england-data}.

\begin{table}
\caption{Summary of the top N vcfzarr columns produced for 
Genomics England aggV2 VCFs on chromosome 20 using \texttt{vcf2zarr}
default settings. Each field is stored independently 
as Zarr array with the given type (sufficient to represent all values in the
...
\midrule
call\_AD &  int16 & 179.0G & 26 & 24.1\\
call\_GQ &  int16 & 171.0G & 13 & 22.2 \\
call\_DP &  int16 & 141.0G & 20 & 18.8 \\
call\_FT &  string & 58.0G & 160 & 7.3 \\
call\_PL &  int16 & 51.0G & 140 & 6.8 \\
call\_GQX &  int16 & 12.0G & 190 & 1.6 \\
call\_genotype & int8 & 6.1G & 380 & 0.8 \\
call\_genotype\_mask & bool & 3.6G  & 630 & 0.48\\
call\_genotype\_phased & bool & 3.6G  & 1700 & 0.48 \\
...
size dominated by a few columns, with call\_AD and call\_GQ consuming 
almost half of total. These fields are much less compressible
than genotype data (which uses $<1\%$ of the total space here)
due to their inherent noisiness~\citep{lin2020sparse}. Substantial
...
Calculating the histogram of call\_AD values at this 
...
computation over a cluster of X workers. The elapsed time 
was X minutes, and requires X lines of Python in a 
