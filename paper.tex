...
from the middle of the set of 106 files for chromosome 2.
...
VCF file, we read in the entire variant\_position array and 
...
may solve this problem by either storing a combined variant\_contig,
variant\_position array as a single chunk, or use a specialised
index that supports overlap queries.
...
\texttt{bcftools +af-dist} as before. 
...
% 716.28 / 2.32 = 308.74
% 716.3 / 4.25 = 168.54
with a total CPU time of 2.32 min and elapsed time of 4.25 min.
...
Xarray~\citep{hoyer2017xarray} provide powerful abstractions
for large-scale data analysis and can be
used to perform interactive analysis
of large VCF Zarr datasets,
with computations transparently distributed over a cluster.
As shown in our benchmarks, lower-level computations
in which optimised kernels are applied on a chunk-by-chunk
...
and Bill \& Melinda Gates Foundation (INV-001927; Sanger subaward G3594).
AM acknowledges support from the Bill \& Melinda Gates Foundation 
(INV-001927).
