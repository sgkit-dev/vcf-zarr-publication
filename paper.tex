...
and compact than PLINK, compressing data using specialised
genotype compression methods~\cite{sambo2014compression}.
...
row-wise, as are the majority of proposed alternative storage 
formats.
...
a subset of 10,000 variants $\times$ 10 samples from the middle of the matrix
...
solutions have been suggested that can be 
adapted~\cite{poterba2024scalable}).
...
unstructured data often included in VCF INFO 
...
developed to store data from the \textit{Anopheles gambiae} 1000 Genomes Project~\citep{anopheles2017genetic}.
A successor project, the MalariaGEN Vector Observatory,
currently uses Zarr to store data from whole-genome sequencing of 23,000 \textit{Anopheles} mosquitoes from 31 African countries.\footnote{\url{https://www.malariagen.net/malaria-vector-genome-observatory/}}
These data are hosted in Google Cloud Storage and can be analysed interactively
...
where access to local computing infrastructure and sufficient network bandwidth to download large datasets may be limited.
...
and good decoding speed makes zstd a good default choice.
